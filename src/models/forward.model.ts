...
export class Forward {
  ...
  async guardar(transaction?: mssql.Transaction) {
    ...
    try {
      ...
      let calendario: CalendarioCierre = new CalendarioCierre({
        ano: this.fechaoperacion.getFullYear(),
        periodo: this.fechaoperacion.getMonth() + 1
      });
  ...
  async procesarCierre(cierreForward: CierreForward) {
    ...
    try {
      ...
      let isOpen: boolean = await new CalendarioCierre().validar(
        transaction,
        cierreForward.ano,
        cierreForward.periodo,
        'registro'
      );
      ...
      result = await new ForwardSaldos().actualizarByAnoAndPeriodo(
        transaction,
        cierreForward.ano,
        cierreForward.periodo,
        cierreForward.id
  ...
  async obtenerTrx(
    ...
    id?: number
  ): Promise<{ ok: boolean; data?: Forward; message?: string }> {
    return new Promise((resolve) => {
      transaction
        .request()
  ...
  private validarCierre(forward: Forward): string[] {
    ...
    forward?.creditos.forEach((credito) => {
      if (credito.saldoasignacion != 0)
        error.push(`El forward tiene un saldo ($${credito.saldoasignacion}) asociado al crédito ${credito.id}`);
    });
    return error;
  }
}
