    ...
    return new Promise(async (resolve) => {
      try {
        ...
        for (let i = 0; i < pagos.length; i++) {
          ...
          if (detallePago.tipopago === 'Capital')
          ...
          if (detallePago.idforward != 0) {
            ...
            await new ForwardSaldos().actualizarByAnoAndPeriodo(
              transaction,
        ...
        await new CreditoSaldos().actualizarByAnoAndPeriodo(
          transaction,
          fechaPago.getFullYear(),
          fechaPago.getMonth() + 1,
          pagos[0].idcredito
  ...
  async procesarReverso(nick: string): Promise<{ ok: boolean; message: any }> {
    ...
    return new Promise(async (resolve) => {
      try {
        await transaction.begin();
        let calendario: CalendarioCierre = new CalendarioCierre({
          ano: this.fechapago.getFullYear(),
          periodo: this.fechapago.getMonth() + 1
        });
        calendario = (await calendario.get(transaction))?.calendario || new CalendarioCierre();
        if (!calendario.registro) throw new Error('El mes se encuentra cerrado para registros.');
        let result = await this.reversar(transaction, nick);
        if (!result.ok) throw new Error('Error al reversar detallepago');
        if (this.tipopago === 'Capital')
          await new Credito().actualizarSaldo(this.idcredito, this.valor * -1, transaction);
        if (this.idforward != 0) {
          ...
          await new ForwardSaldos().actualizarByAnoAndPeriodo(
            transaction,
            this.fechapago.getFullYear(),
            this.fechapago.getMonth() + 1,
            this.idforward
          );
        }
        result = await new CreditoSaldos().actualizarByAnoAndPeriodo(
          transaction,
          this.fechapago.getFullYear(),
          this.fechapago.getMonth() + 1,
          this.idcredito
