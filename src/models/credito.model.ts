...
export class Credito {
  ...
  async guardar(transaction?: mssql.Transaction) {
    ...
    try {
      ...
      let calendario: CalendarioCierre = new CalendarioCierre({
        ano: this.fechadesembolso.getFullYear(),
        periodo: this.fechadesembolso.getMonth() + 1
      });
  ...
  async obtener(transaction?: mssql.Transaction): Promise<{ ok: boolean; data?: Credito; message?: any }> {
    return new Promise(async (resolve) => {
      ...
      try {
        if (!isTrx) await transaction.begin();
        let result = await transaction.request().input('id', mssql.Int(), this.id).execute('sc_credito_obtener');
        if (!isTrx) {
          transaction.commit();
          pool.close();
        }
  ...
  async obtenerTrx(
    ...
    id?: number
  ): Promise<{ ok: boolean; data?: Credito; message?: string }> {
    return new Promise((resolve) => {
      transaction
        .request()
      ...
      pool
        ...
        .then((result) => {
          ...
          if (result.recordset[0]?.length > 0) {
            ...
            resolve({
              ok: false,
              message: `El pagaré ${credito.pagare} ya se encuenta asociado a la obligacion #${credito.id}`
            });
          }
  ...
  async simular365() {
    ...
    else if (this.indexado.descripcion == 'UVR')
      tasa = await macroeconomico.getTasaUVR(this.fechadesembolso, this.fechadesembolso);
    ...
    for (let i = 1; i <= this.plazo; i++) {
      ...
      if (this.indexado.descripcion == 'UVR')
        tasa = await macroeconomico.getTasaUVR(this.fechadesembolso, fechaPeriodo);
      ...
      (amortizacion.tasaEA =
        this.indexado.descripcion == 'UVR' ? tasa / 100 + spreadEA : (1 + tasa / 100) * (1 + spreadEA) - 1),
  ...
  async simular365Trx(transaction: mssql.Transaction) {
    ...
    if (this.indexado.descripcion == 'TASA FIJA') tasa = this.tasa;
      ...
      tasa = await macroeconomico.getTasaUVRTrx(transaction, this.fechadesembolso, this.fechadesembolso);
  ...
  private calcularInteresPagado(ncuota: number, tasaEA: number): number {
    ...
    if (this.amortizacionint.config.nper === -1)
      return ncuota === this.plazo ? tasaEM * this.amortizacion[ncuota - 1].saldoCapital * this.plazo : 0;
    if (ncuota % this.amortizacionint.config.nper === 0)
      return tasaEM * this.amortizacion[ncuota - 1].saldoCapital * this.amortizacionint.config.nper;
    return 0;
  }
  ...
  private calcularAbonoCapital(ncuota: number): number {
    ...
    if (ncuota % this.amortizacionk.config.nper === 0)
      return (
        (this.amortizacion[ncuota - 1].saldoCapital / (this.plazo - ncuota + this.amortizacionk.config.nper)) *
        this.amortizacionk.config.nper
      );
    return 0;
  }
